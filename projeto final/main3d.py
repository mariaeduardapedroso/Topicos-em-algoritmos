...
def grasp(distances, max_iter, alpha):
    best_route = None
    ...
    for _ in range(max_iter):
        ...
        while remaining:
            ...
            rcl = []
            max_dist = max(distances[current, point] for point in remaining)
            threshold = alpha * max_dist
            for point in remaining:
                if distances[current, point] <= threshold:
                    rcl.append(point)
    ...
    while improved:
        ...
        for i in range(1, len(route) - 1):
            for j in range(i + 1, len(route)):
                if j - i == 1: continue  # Evitar troca de vizinhos consecutivos
...
def load_star_coordinates(filename):
    with open(filename, 'r') as file:
        ...
        coordinates = np.array([list(map(float, line.split())) for line in lines])
    return coordinates

# Carregar as coordenadas das estrelas
star_coordinates = load_star_coordinates('star100.xyz')
